...
const CanvasNew = (props) => {
    const [fadePen,setFadePen]=useState('secondary');
    const [fadeBoxBtn,setFadeBoxBtn]=useState('secondary');
  ...
  const addTextBox = (linkId,textBoxDict,textBoxPageIndex) => {
    if(textBoxPageIndex==undefined){
      ...
      setTextboxes(prevState =>
        ...
        setFadePen('secondary');
        setFadeBoxBtn('dark');
    
  };

  const handleDragChange = (type, id, text,e) => {
    e.stopPropagation()
    if (type == "note") {
      setNotes(
  ...
  return (
    <div style={{background: "azure",overflow:"scroll" }} onClick={ ()=>setShowPicker(false)}>
      <Toolbar 
      ...
      <div style={{ position: "relative" }}>
        ...
        {pdfHighLightedText?.map((pdfHighLightedText, index) => (
          ...
          > 
            <div  className="card card-body rounded-3 p-0" onChange={(e) => handleDragChange("pdfExtractedText", pdfHighLightedText.id, e.target.value,e)} style={{width: "95%", height: "100%", backgroundColor: `${pdfHighLightedText.color}`, border: "none", resize: "none", outline: "none", overflow: "hidden",fontSize:"16px"}}>
            <div className="input-group input-group-sm">
              <em style={{background: 'none',
                        border: 'none'}} title={`To Page no. ${pdfHighLightedText.argIndex+1}`} className="text-primary input-group-text p-1" onClick={(e) =>goToAnnoPageLinkage(e, pdfHighLightedText.linkingId,pdfHighLightedText.argIndex)}>
              ...
              <em style={{cursor:"pointer",background: 'none',border: 'none'}} className="fa-solid fa-paintbrush text-lg input-group-text p-1" onClick={(e)=> handleChooseColor(e,index,'pdfTextCard')} ></em>
            </div>
            <span className="">
            ...
            <div className="d-flex justify-content-end fixed-bottom">
              <div className="input-group input-group-sm ">
              <em style={{background: 'none', border: 'none'}}  title={`To Page no. ${pdfHighLightedText.rightArgIndex?pdfHighLightedText.rightArgIndex+1:"No Link"}`} className="text-primary input-group-text p-1"  onClick={(e)=> linkOrGotoAnnotatedpage(e, pdfHighLightedText.rightLinkId,pdfHighLightedText.linkingId,pdfHighLightedText.rightArgIndex)}>
              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" cursor={"pointer"}  viewBox="0 0 16 16">
                  <path d="M8 0a8 8 0 1 0 0 16A8 8 0 0 0 8 0m3.5 7.5a.5.5 0 0 1 0 1H5.707l2.147 2.146a.5.5 0 0 1-.708.708l-3-3a.5.5 0 0 1 0-.708l3-3a.5.5 0 1 1 .708.708L5.707 7.5z" />
                </svg>
        ...
        {textboxes?.map((ele, index) => (
          ...
          >
            ...
            <div className="input-group input-group-sm">
              <em style={{background: 'none', border: 'none'}} title={`To Page no. ${ele.pageIndex?ele.pageIndex+1:"No Linkage"}`} className="text-primary input-group-text p-1" onClick={(e) =>goToAnnoPageLinkageOfTextBox(e, ele.pageIndex)}>
              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" cursor={"pointer"}  viewBox="0 0 16 16">
                <path d="M8 0a8 8 0 1 0 0 16A8 8 0 0 0 8 0m3.5 7.5a.5.5 0 0 1 0 1H5.707l2.147 2.146a.5.5 0 0 1-.708.708l-3-3a.5.5 0 0 1 0-.708l3-3a.5.5 0 1 1 .708.708L5.707 7.5z" />
              </svg>
              <p className={`badge badge-sm ${ele.pageIndex?'bg-success':'bg-warning'} m-1`}>{ele.pageIndex ? ele.pageIndex+1 :""}</p>
              </em>
              <em  style={{cursor:"pointer",background:"none",border:"none"}} className="fa-solid fa-paintbrush text-lg input-group-text p-1" onClick={(e)=> handleChooseColor(e,index,'textBoxCard')}></em>
            </div>               
